...
class SendInBlue extends ServiceInterface {
	...
	public function createContact( Contact $contact ) {
		...
		do_action(
			'log', 'debug', 'SendinBlue createContact() invoked',
			array(
				'request' => $contact_attributes,
			)
		);
		...
		try {
			...
			do_action(
				'log', 'error', 'Sendinblue createContact() API request error',
				array(
					'error' => $error,
				)
			);
			...
			do_action(
				'log', 'error', 'Sendinblue createContact() API request general error',
				array(
					'error' => $e->getMessage(),
				)
			);
		...
		do_action(
			'log', 'info', 'Sendinblue createContact() successful',
			array(
				'result' => $raw_result,
			)
		);
	...
	public function getContact( $search_contact ) {
		...
		try {
			...
			if ( 'document_not_found' === $error->code ) {
				do_action(
					'log', 'notice', 'Sendinblue getContact() API contact not found',
					array(
						'contact' => $contact,
					)
				);
			...
			do_action(
				'log', 'error', 'Sendinblue getContact() API request error',
				array(
					'error' => $error,
				)
			);
			...
			do_action(
				'log', 'error', 'Sendinblue getContact() API request general error',
				array(
					'error' => $e->getMessage(),
				)
			);
		...
		do_action(
			'log', 'info', 'Sendinblue getContact() API request successful',
			array(
				'result' => $raw_result,
			)
		);
	...
	public function saveContact( Contact $contact, $custom_attributes = array() ) {
		...
		do_action(
			'log', 'debug', 'SendinBlue saveContact() invoked',
			array(
		...
		try {
			...
			if ( 'document_not_found' === $error->code ) {
				do_action(
					'log', 'notice', 'Sendinblue saveContact() API contact not found',
					array(
						'contact' => $contact,
					)
				);
			...
			do_action(
				'log', 'error', 'Sendinblue saveContact() API request error',
				array(
					'error' => $error,
				)
			);
		...
		do_action(
			'log', 'info', 'Sendinblue saveContact() API request successful',
			array(
				'result' => $raw_result,
			)
		);
	...
	public function getLists( $limit = 10, $offset = 0 ) {
		...
		if ( false === $lists ) {
			...
			try {
				...
				do_action(
					'log', 'error', 'SendinBlue getLists() request error',
					array(
						'error' => $e->getMessage(),
					)
				);
			...
			do_action(
				'log', 'info', 'SendinBlue getLists() successful retreive',
				array(
					'result' => $list_result,
				)
			);
			...
			do_action(
				'log', 'debug', 'SendinBlue getLists() loaded from cache',
				array(
					'lists' => $lists,
				)
			);
